...
# alternatively rssread must be installed
type rssread.py >/dev/null 2>&1 || {
	...
	type rssread.py >/dev/null 2>&1 || {
		echo "This script requires that you install the script \"rssread.py\" on the server."
		exit 0
	}
}
...
function readrss() {
	...
	$TORIFY rssread.py "$1" $2 | sed 'H;1h;$!d;x; s/\n\n\n\n\n/\n\n/g' | sed 'H;1h;$!d;x; s/\n\n\n\n/\n\n/g' |
		sed 'H;1h;$!d;x; s/\n\n\n/\n\n/g' | sed '/Pub.date: /a \\n\n' # add newlines for separation after last line
}
